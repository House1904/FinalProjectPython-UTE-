...
def heatmap():
    ...
    fig, ax = plt.subplots(figsize=(8, 6))  # Điều chỉnh kích thước của heatmap
    sns.heatmap(corr_df, annot=True, cmap='coolwarm', fmt=".2f", ax=ax)  # Vẽ heatmap, hiện giá trị tương quan, và sử dụng bảng màu "coolwarm"
    ax.set_title('Biểu đồ Heatmap của các cột số')  # Thêm tiêu đề
    return fig  # Hiển thị biểu đồ